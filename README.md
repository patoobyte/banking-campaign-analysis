...
1. **DEMO - Dataset builder** previews edits to its URL list before capture. **Save draft and capture missing pages** saves the draft first, routes supported URLs to their actual bank (even when pasted under another bank), and reuses cached pages with both text and screenshots. It shows draft and per-bank missing counts; reload the saved list if another session has changed it. It captures rendered text, full-page screenshots, DOM counts, and screenshot metrics without a sitemap or eligibility model. The URL source `.txt` files are not updated by the demo editor.
   **Manual ING youth capture:** The live English youth-account campaign renders black in the browser here. Its saved HTML, six top-to-bottom user screenshots, and a manually reviewed screenshot OCR transcript are registered under `https://www.ing.be/en/individuals/campaign/current-account-free-youth`. The screenshots are supplied individually at original resolution to the coding request; its HTML quantitative metrics describe only the saved JavaScript shell and must not be treated as rendered page counts. Superseded youth URL captures and coded records are archived under `data/demo/archives/` and removed from active demo indexes/run JSON. This replacement is deliberately **not pre-coded**: select ING in Campaign run and leave "Recode all" unchecked to code only pending/error campaigns. The six user-provided PNGs/HTML and reviewed text are under `Filteredurl/Blockedurl/Ing/` and the replacement URL is in `Filteredurl/INGcampainurl.txt`; if ignored `data/demo/` is absent in a fresh checkout, capturing ING reconstructs this manual entry from those sources before opening the browser for any other missing pages. Keep the ignored `data/demo/archives/` separately to retain the old records; the manual capture must never be replaced by live browser scraping.
...
3. **DEMO - Overview** compares Left and Right campaigns or banks using compact, bounded-width radar and categorical charts with identifiable legends. Categorical labels are not 1-5 scores: individual-campaign charts show label presence (Yes/No) and bank charts show the percentage of eligible campaigns carrying each label. Numerical metrics each have an independent scale: bars show individual values or whole-bank means; box plots show bank distributions and coloured value markers for single campaigns. Up to three metrics can be charted at once. Missing metrics are not interpreted as zeros.
...
Visual and profile categories use controlled short labels with separate evidence fields for grouping and charts. Neutral light/dark-mode backgrounds are ignored when classifying the primary brand/imagery accent. Previously coded prose remains unchanged in JSON; CSV leaves its label cells empty while retaining the original prose as evidence. Recode a selected bank with the current prompt to generate graphable labels (this makes new AI calls using the cached URL-specific screenshots).
